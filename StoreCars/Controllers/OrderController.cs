...
using StoreCars.Models.ViewModel;
using StoreCars.Services;
...
namespace StoreCars.Controllers
{
    public class OrderController : Controller
    {
        ...
        public OrderController(DataCarContext context, INotyfService notyfService,IVnPayService vnPayService)
        {
        ...
        public async Task<IActionResult> BuyNow(int id)
        {
            ...
            // Truy vấn chi tiết đơn hàng để kiểm tra xe
            var chitietdonthang = await _context.OderItems.FirstOrDefaultAsync(x => x.OderId == dondathang.OdersId && x.ProductId == id);

            if (chitietdonthang == null)
            {
                // Nếu sách chưa có trong chi tiết đơn hàng, thêm mới xe vào chi tiết đơn hàng
                var sach = await _context.Products.FirstOrDefaultAsync(x => x.ProductId == id);
                chitietdonthang = new OderItem
                {
                    OderId = dondathang.OdersId,
            ...
            {
                // Xe đã có trong chi tiết đơn hàng, tăng số lượng lên một đơn vị
                chitietdonthang.Quantity++;
                await _context.SaveChangesAsync();
            }
            _notyfService.Success("Thêm sản phẩm thành công ");
        ...
        public async Task<IActionResult> CheckOut(int tongtien, int address, string maGiamGia, int pay, int Shiper)
        {
            ...
            {
                var discount = await _context.Discounts.FirstOrDefaultAsync(x => x.Code == maGiamGia);
                if (discount == null || discount.UseNumber >= discount.Quantity)
                {
                    _notyfService.Error("Mã giảm giá không tồn tại hoặc đã hết lượt sử dụng");
                    return RedirectToAction("Index");
                }

                // Cập nhật số lượng sử dụng và áp dụng giảm giá
                discount.UseNumber += 1; // Tăng số lượng sử dụng
            ...
            decimal ship = (Shiper == 1) ? 30000m : 45000m;
