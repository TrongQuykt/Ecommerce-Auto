...
namespace StoreCars.Areas.Admin.Controllers
{
    ...
    public class RevenueController : Controller
    {
        ...
        public RevenueController(DataCarContext context, INotyfService notyfService)
        {
            _context = context;
        ...
        public async Task< IActionResult> Index()
        {
            ...
            decimal? doanhthucuahang = DoanhThu.Sum(x => x.Total);


            ViewBag.DonHuy = DonHuy.Count();
            ViewBag.TongDonHang = DonHang.Count();
            ViewBag.TongTien = doanhthucuahang;
        ...
        public IActionResult ExportExcelNgay(DateTime TuNgay, DateTime DenNgay)
        {
            try
            {
                ...
                var data = (from order in _context.Oders
                            where order.Status != 1 && (order.Status != 6 || order.Status == null) && (order.CreateDay >= TuNgay && order.CreateDay <= DenNgay)
                            select new
                            {
        ...
        public IActionResult ExportExcel()
        {
            try
            {
                var data = (from order in _context.Oders
                            ...
                            where order.Status != 1 && (order.Status != 6 || order.Status == null)
