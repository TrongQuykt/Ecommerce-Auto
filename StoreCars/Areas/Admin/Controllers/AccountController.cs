...
namespace StoreCars.Areas.Admin.Controllers
{
    ...
    {
        private DataCarContext _context; 
        public static string? image;
        public INotyfService _notyfService { get; }
        private readonly IConfiguration _configuration;
        public AccountController(DataCarContext repo, INotyfService notyfService, IConfiguration configuration)
        {
            _context = repo;
            _notyfService = notyfService;
            _configuration = configuration;
        }
        [Authorize(Roles = "Admin, NhanVien, Manager")] 
        ...
        public async Task<IActionResult> Create(Account account, IFormFile fAvatar)
        {
            ...
            account.Avartar = "/contents/images/istockphoto-693203680-612x612 (1).png";
        ...
        public async Task<IActionResult> DeleteConfirmed(int id)
        {
            ...
            {
                return Problem("Entity set 'DataCarContext.Accounts'  is null.");
            }
        ...
        public async Task<IActionResult> Login(string user, string pass)
        {
            ...
            else if (account.RoleId == 1) // Nếu vai trò là admin
            {
                _notyfService.Warning("Tài khoản bạn là tài khoản Admin");
            }
            else if (account.RoleId == 2) // Nếu vai trò là nhân viên
            {
                _notyfService.Warning("Tài khoản bạn là tài khoản Nhân Viên");
            }
            else if (account.RoleId == 4) // Nếu vai trò là quản lý (Manager)
            {
                _notyfService.Warning("Tài khoản bạn là tài khoản Manager");
            }
            if (account != null)
            {
                ...
                List<Claim> claims = new List<Claim>()
                    {
                        ...
                        account.RoleId == 1 ? new Claim(ClaimTypes.Role, "Admin") : new Claim(ClaimTypes.Role, "NhanVien") , new Claim(ClaimTypes.Role, "Manager"),
